...
This run proposes **both equity gaps and option trades** — option trades being covered-call rolls and opening covered-call sales. They are selected by different rules — STEP 3 for equity, STEP 3B for options — and travel in the same proposal batch.
...
- `gaps` is equity. `optionGaps` is the priced option plans — each with `underlying`, `planKind` (`"roll"`, `"open"`, or null), `rollType`, `contracts`, `close`, `open`, `pricing`, `coverage`, `proposable`, `reasons[]`, and a ready-to-submit `order`. (`options` is the raw, unpriced divergence list that `optionGaps` is derived from — it is the morning report's field, not this task's. Never build an order from `options`.)
...
STEP 3B — Option rolls AND opening covered-call sales:
- Work from `optionGaps` where `proposable` is true. Ignore `coverageRows` — those are the morning report's job.
- **Two shapes are proposable, and `planKind` says which.** Submit both:
  - `planKind: "roll"` — a net-priced two-leg covered-call roll (`BUY_TO_CLOSE` + `SELL_TO_OPEN`, `NET_CREDIT`/`NET_DEBIT`/`NET_ZERO`, with a real `complexOrderStrategyType`).
  - `planKind: "open"` — nothing to close, one call sold to open, as a **single-leg `LIMIT` at the bid with NO `complexOrderStrategyType`**. This is the benchmark holding a short call Partnership has not written yet. It is a covered call, not a naked one: the coverage gate below is what makes that true, and it is checked before the entry is ever marked proposable.
  - A bare CLOSE (Partnership holds a short call CRT does not) is deliberately NOT proposable. Report it and leave it.
- **Option `proposable` is stricter than equity `proposable` and needs no guards of your own.** For BOTH shapes it already folds in the trade shape, post-trade share coverage (only shares cover; long calls never do), expiry sanity, and full quote health — **including the per-symbol halt check** (`status` not "Normal" → a reason), crossed or too-wide quotes, and for an opening sale a bid of zero. Do not re-derive any of it and do not override it. The equity halt guard in STEP 3 does not apply here; applying it twice is harmless, but skipping an entry that is already `proposable` is not.
- There is **no notional floor for options** — an option divergence is a whole position, not rounding noise. A $94 roll qualifies where a $94 equity gap would not, and so does a $65 opening sale.
- **Copy the entry's `order` object VERBATIM into the proposal.** Do not rebuild it, re-price it, reorder its legs, or change `orderType`, `price`, or `complexOrderStrategyType`. The two shapes have opposite requirements on that last field and the CLI has already set it correctly: a net price REQUIRES a real `complexOrderStrategyType` (`VERTICAL`/`CALENDAR`/`DIAGONAL`) — with `NONE` Schwab reads `price` as a plain limit price and rejects the order — while a single-leg `LIMIT` must NOT carry one at all, since naming a strategy is what makes Schwab read a one-leg order as net-priced. Copying verbatim is what keeps both right; rebuilding by hand is what got one of them wrong before.
- **Never split a roll into two single-leg orders.** A roll is one net-priced two-leg order. Two orders can half-fill, and inverted they leave the account briefly short an uncovered call. One `optionGaps` entry = exactly one order in the batch, for either `planKind`.
- For every `optionGaps` entry where `proposable` is false, report the underlying and its `reasons[]` verbatim in your output summary, so a report-only divergence is visibly deferred rather than silently dropped. Do NOT try to make it proposable, and in particular do NOT hand-build an order for it from the snapshot — if the CLI would not price it, this task does not trade it.

- If neither STEP 3 nor STEP 3B yields anything, report "no qualifying equity gaps or option trades" and STOP without submitting anything.
...
- Write the proposal JSON to /tmp/drift-proposal.json (this runs on the host Mac — /tmp is writable):
  { "summary": "<one-paragraph drift summary: per-symbol share deltas vs CRT, plus each option roll or opening sale in words>", "accountNumber": "13102970", "orders": [ ... ] }
...
- Describe each option trade in the `summary` in plain words — expiry, strike(s), direction, price, and the post-trade coverage from `coverage` (shares vs required). For a `planKind: "open"` entry, say plainly that it OPENS a new covered call rather than rolling an existing one, and give the shares backing it. A raw OCC symbol does not read as a date and a strike to whoever is approving it. The worker renders the legs legibly in Slack too, but the summary is what frames it.
- Check the buy side against Partnership `cashBalance`. Count an option trade's cash effect as its `pricing.notional` when `pricing.direction` is "debit", and as zero when it is a credit (an opening covered-call sale is always a credit, so it never consumes cash). If total buy notional exceeds available cash, still submit, but state the shortfall prominently in the `summary` field so the approver sees it before approving. Describe it only as a shortfall against the reported cash figure — do NOT assert how it would be funded, and in particular do not call it a margin borrow.
...
OUTPUT: a short summary — the market session, which equity gaps qualified, which option rolls and opening covered-call sales qualified (say which `planKind` each was), any equity symbols excluded for a non-Normal quote status, any option divergence that was report-only with its reasons, and the proposalId if one was created (or the reason none was).
